...
class OpenSpielEnvironmentLoop(core.Worker):
        ...
        if player == pyspiel.PlayerId.TERMINAL:
            for player_id in range(len(self._actors)):
                ...
                if self._observed_first[player_id]:
                    ...
                    self._actors[player_id].observe(
                        self._prev_actions[player_id], player_timestep
                    )
            ...
            if not self._observed_first[player]:
                ...
                self._actors[player].observe(
                    self._prev_actions[player], player_timestep
                )
                if self._should_update:
    ...
    def _get_player_timestep(
        self, timestep: dm_env.TimeStep, player: int
    ) -> dm_env.TimeStep:
    ...
    def run_episode(self) -> loggers.LoggingData:
        ...
        episode_return = tree.map_structure(
            _generate_zeros_from_spec, multiplayer_reward_spec
        )
