...
def init_default_network(
    agent_type: DefaultSupportedAgent, agent_spec: specs.EnvironmentSpec
) -> ma_types.Networks:
...
def init_default_policy_network(
    ...
    eval_mode: ma_types.EvalMode = False,
) -> ma_types.PolicyNetwork:
    """Returns default policy network for a single agent."""
    if agent_type == DefaultSupportedAgent.TD3:
        sigma = 0.0 if eval_mode else config.sigma
        return td3.get_default_behavior_policy(network, agent_spec.actions, sigma=sigma)
    elif agent_type == DefaultSupportedAgent.SAC:
...
def init_default_config(
    agent_type: DefaultSupportedAgent, config_overrides: Dict[str, Any]
) -> ma_types.AgentConfig:
...
def policy_network_factory(
    ...
    for agent_id, agent_type in agent_types.items():
        single_agent_spec = ma_utils.get_agent_spec(environment_spec, agent_id)
        policy_networks[agent_id] = init_fn(
            agent_type,
            networks[agent_id],
            single_agent_spec,
