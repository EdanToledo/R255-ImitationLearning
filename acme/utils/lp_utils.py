...
def make_xm_docker_resources(program, requirements: Optional[str] = None):
    ...
    """
    if FLAGS.lp_launch_type != "vertex_ai" and FLAGS.lp_launch_type != "local_docker":
    ...
    if "learner" in num_nodes:
        ...
        xm_resources["learner"] = lp.DockerConfig(
            acme_location,
            requirements,
            hw_requirements=xm.JobRequirements(cpu=learner_cpu, ram=6 * xm.GiB, P100=1),
            python_path=python_path,
        )
    ...
    if "environment_loop" in num_nodes:
        xm_resources["environment_loop"] = lp.DockerConfig(
            acme_location,
            requirements,
            hw_requirements=xm.JobRequirements(cpu=6, ram=6 * xm.GiB, P100=1),
            python_path=python_path,
        )
