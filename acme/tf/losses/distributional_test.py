...
def _reference_l2_project(src_support, src_probs, dst_support):
    ...
    common_shape = np.broadcast(src_support[0, ..., 0], dst_support[..., 0]).shape
    ...
    src_support = np.broadcast_to(
        src_support, src_support.shape[:1] + common_shape + src_support.shape[-1:]
    )
    src_probs = np.broadcast_to(src_probs, src_support.shape)
    dst_support = np.broadcast_to(dst_support, common_shape + dst_support.shape[-1:])

    output_shape = (src_support.shape[0],) + dst_support.shape

    # Collapse all but the first (batch) and last (atom) axes.
    src_support = src_support.reshape([src_support.shape[0], -1, src_support.shape[-1]])
    src_probs = src_probs.reshape([src_probs.shape[0], -1, src_probs.shape[-1]])

    # Collapse all but the last (atom) axes.
    dst_support = dst_support.reshape([-1, dst_support.shape[-1]])

    dst_probs = np.zeros(
        src_support.shape[:1] + dst_support.shape, dtype=src_probs.dtype
    )
...
class L2ProjectTest(parameterized.TestCase):
    ...
    def test_multiaxis(self, src_shape, dst_shape):
        ...
        dst_support = make_support(dst_shape, -0.75)
        ...
        expected_dst_probs = _reference_l2_project(src_support, src_probs, dst_support)
        ...
        dst_probs = distributional.multiaxis_l2_project(
            ...
            tf.convert_to_tensor(dst_support),
        ).numpy()
    ...
    @parameterized.parameters(
        ...
        (
            ...
            [[0.0, 0.5, 0.5, 0]],
            [0.25, 1.25, 2.25, 3.25],
            0.5 * (np.array([[0.25, 0.75, 0, 0]]) + np.array([[0.0, 0.25, 0.75, 0]])),
        ),
