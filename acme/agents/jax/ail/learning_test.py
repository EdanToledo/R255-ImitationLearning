...
def _make_discriminator(spec):
    def discriminator(*args, **kwargs) -> networks_lib.Logits:
        return ail_networks.DiscriminatorModule(
            ...
            use_action=False,
            use_next_obs=False,
            network_core=ail_networks.DiscriminatorMLP([]),
        )(*args, **kwargs)
    ...
    return ail_networks.make_discriminator(
        environment_spec=spec, discriminator_transformed=discriminator_transformed
    )
...
class AilLearnerTest(absltest.TestCase):
    def test_step(self):
        ...
        spec = specs.EnvironmentSpec(simple_spec, simple_spec, simple_spec, simple_spec)
        ...
        zero_transition = types.Transition(
            np.array([0.0]), np.array([0.0]), 0.0, 0.0, np.array([0.0])
        )
        zero_transition = utils.add_batch_dim(zero_transition)

        one_transition = types.Transition(
            np.array([1.0]), np.array([0.0]), 0.0, 0.0, np.array([0.0])
        )
