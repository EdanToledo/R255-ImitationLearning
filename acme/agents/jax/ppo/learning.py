...
class PPOLearner(acme.Learner):
    ...
    def __init__(
        ...
        def sgd_step(state: TrainingState, minibatch: Batch):
            ...
            params = PPOParams(
                model_params=model_params, num_sgd_steps=state.params.num_sgd_steps + 1
            )
        ...
        def epoch_update(
            ...
            permuted_batch = jax.tree_util.tree_map(
                lambda x: jax.random.permutation(  # pylint: disable=g-long-lambda
                    sub_key, x, axis=0, independent=False
                ),
                carry_batch,
            )
        ...
        def single_device_update(
            ...
            if normalize_advantage or normalize_value:
                ...
                zero_debias = 1.0 / (
                    1.0 - jnp.power(normalization_ema_tau, ema_counter)
                )
            ...
            if max_abs_reward is not None:
                ...
                rewards = jnp.clip(rewards, -1.0 * max_abs_reward, max_abs_reward)
            discounts = termination * discount
            behavior_log_probs = extra["log_prob"]
            _, behavior_values = vmapped_network_apply(
                state.params.model_params, observations
            )
            ...
            if normalize_value:
                ...
                biased_value_first_moment = (
                    ...
                    + (1.0 - normalization_ema_tau) * batch_value_first_moment
                )
                biased_value_second_moment = (
                    normalization_ema_tau * state.biased_value_second_moment
                    + (1.0 - normalization_ema_tau) * batch_value_second_moment
                )
                ...
                behavior_values = (
                    behavior_values * jnp.fmax(state.value_std, 1e-6) + state.value_mean
                )
            ...
            if normalize_advantage:
                ...
                batch_advantage_scale = jax.lax.pmean(
                    batch_advantage_scale, pmap_axis_name
                )

                # update the running statistics
                biased_advantage_scale = (
                    normalization_ema_tau * state.biased_advantage_scale
                    + (1.0 - normalization_ema_tau) * batch_advantage_scale
                )
                ...
                scaled_advantages = batch.advantages / jnp.fmax(
                    state.advantage_scale, 1e-6
                )
        ...
        pmapped_update_step = jax.pmap(
            single_device_update, axis_name=pmap_axis_name, devices=self.learner_devices
        )
        ...
        def make_initial_state(key: networks_lib.PRNGKey) -> TrainingState:
            ...
            initial_params = jax.device_put_replicated(
                initial_params, self.local_learner_devices
            )
            initial_opt_state = jax.device_put_replicated(
                initial_opt_state, self.local_learner_devices
            )
            ...
            ema_counter = jax.device_put_replicated(
                ema_counter, self.local_learner_devices
            )
            ...
            if normalize_obs:
                obs_norm_params = (
                    obs_normalization_fns.init()
                )  # pytype: disable=attribute-error
                obs_norm_params = jax.device_put_replicated(
                    obs_norm_params, self.local_learner_devices
                )
                init_state = init_state._replace(
    ...
    def restore(self, state: TrainingState):
        ...
        random_key = jax.random.split(random_key, num=self.num_local_learner_devices)
