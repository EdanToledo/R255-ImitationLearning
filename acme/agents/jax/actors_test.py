...
class ActorTest(parameterized.TestCase):
    @parameterized.named_parameters(("policy", False), ("policy_with_extras", True))
    def test_feedforward(self, has_extras):
        ...
        if has_extras:
            ...
            actor_core = actor_core_lib.batched_feed_forward_to_actor_core(policy.apply)
...
class RecurrentActorTest(absltest.TestCase):
    def test_recurrent(self):
        ...
        def network(inputs: jnp.ndarray, state: hk.LSTMState):
            return hk.DeepRNN(
                [hk.Reshape([-1], preserve_dims=1), hk.LSTM(output_size)]
            )(inputs, state)
        ...
        actor = actors.GenericActor(actor_core, jax.random.PRNGKey(1), variable_client)
