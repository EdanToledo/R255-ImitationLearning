...
class VariableClientTest(tf.test.TestCase):
    ...
    def test_update(self):
        ...
        variable_client = tf2_variable_utils.VariableClient(
            variable_source,
            {"policy": self._actor_model.variables},
        ...
        for _ in range(_UPDATE_PERIOD):
            ...
            self.assertNotAllClose(
                self._actor_model.variables, self._learner_model.variables
            )
        ...
        self.assertAllClose(self._actor_model.variables, self._learner_model.variables)
