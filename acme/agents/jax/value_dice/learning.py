...
class ValueDiceLearner(acme.Learner):
    ...
    def __init__(
        ...
        def compute_losses(
            ...
            nu_demo_t0 = networks.nu_network.apply(
                nu_params, demo_o_t0, policy_demo_a_t0
            )
            nu_demo_tm1 = networks.nu_network.apply(nu_params, demo_o_tm1, demo_a_tm1)
            nu_demo_t = networks.nu_network.apply(nu_params, demo_o_t, policy_demo_a_t)
            nu_demo_diff = nu_demo_tm1 - discount * nu_demo_t

            # "Value function" nu over the replay buffer states.
            nu_replay_tm1 = networks.nu_network.apply(
                nu_params, replay_o_tm1, replay_a_tm1
            )
            nu_replay_t = networks.nu_network.apply(
                nu_params, replay_o_t, policy_replay_a_t
            )
            ...
            nu_replay_demo_diff = jnp.concatenate(
                [nu_demo_diff, nu_replay_diff], axis=0
            )
            ...
            non_linear_loss = jnp.sum(
                jax.lax.stop_gradient(
                    utils.weighted_softmax(
                        nu_replay_demo_diff, replay_demo_weights, axis=0
                    )
                )
                * nu_replay_demo_diff
            )
            ...
            if nu_reg_scale > 0.0:
                ...
                nu_grad = jnp.concatenate(
                    [
                        ...
                        jnp.reshape(nu_grad_a, [batch_size, -1]),
                    ],
                    axis=-1,
                )
        ...
        def sgd_step(
            state: TrainingState, data: Tuple[types.Transition, types.Transition]
            ...
            (policy_loss_value, nu_loss_value), vjpfun = jax.vjp(
                compute_losses_with_input, state.policy_params, state.nu_params
            )
