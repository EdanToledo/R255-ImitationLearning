...
def ail_update_step(
    state: DiscriminatorTrainingState,
    ...
    def compute_loss(discriminator_params: networks_lib.Params) -> losses.LossOutput:
...
class AILLearner(acme.Learner):
    ...
    def __init__(
        ...
        direct_rl_iterator = (
            self._process_sample(sample.direct_sample) for sample in direct_rl_iterator
        )
        ...
        self._get_reward = jax.jit(
            functools.partial(ail_networks.compute_ail_reward, networks=ail_network)
        )
    ...
    def _process_sample(self, sample: reverb.ReplaySample) -> reverb.ReplaySample:
        ...
        rewards = self._get_reward(
            self._state.discriminator_params,
    ...
    def step(self):
        ...
        self._state, metrics = self._update_step(
            state, (demo_transitions, rb_transitions)
        )
