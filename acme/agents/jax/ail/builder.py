...
def _split_transitions(
    transitions: types.Transition, index: int
) -> Tuple[types.Transition, types.Transition]:
...
def _rebatch(
    iterator: Iterator[types.Transition], batch_size: int
    ...
    while True:
        while len(data.reward) < batch_size:
            ...
            data = tree.map_structure(
                lambda *args: np.concatenate(list(args)), data, next_data
            )
...
def _mix_arrays(
    replay: np.ndarray, demo: np.ndarray, index: int, seed: int
) -> np.ndarray:
...
def _generate_samples_with_demonstrations(
    ...
    for sample, demos in zip(replay_iterator, demonstration_iterator):
        output_transitions = tree.map_structure(
            functools.partial(_mix_arrays, index=policy_insertion_size, seed=count),
            sample.data,
            demos,
        )
        count += 1
...
class AILBuilder(
    builders.ActorLearnerBuilder[
        ail_networks.AILNetworks, DirectPolicyNetwork, learning.AILSample
    ],
    ...
    def __init__(
        ...
        rl_agent: builders.ActorLearnerBuilder[
            ail_networks.DirectRLNetworks, DirectPolicyNetwork, reverb.ReplaySample
        ],
    ...
    def make_learner(
        ...
        discriminator_optimizer = self._config.discriminator_optimizer or optax.adam(
            1e-5
        )
    ...
    ) -> Iterator[learning.AILSample]:
        batch_size_per_learner_step = ail_config.get_per_learner_step_batch_size(
            self._config
        )

        iterator_demonstration = self._make_demonstrations(batch_size_per_learner_step)
