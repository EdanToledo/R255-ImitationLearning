...
def make_networks(
    ...
    critic_network = snt.Sequential(
        [
            networks.CriticMultiplexer(critic_network=critic_network_base),
...
def make_objectives() -> (
    Tuple[Sequence[mompo.RewardObjective], Sequence[mompo.QValueObjective]]
):
    """Define the multiple objectives for the policy to learn."""
    task_reward = mompo.RewardObjective(name="task", reward_fn=task_reward_fn)
    action_norm = mompo.QValueObjective(
        name="action_norm_q", qvalue_fn=compute_action_norm
    )
...
class MOMPOTest(parameterized.TestCase):
    @parameterized.named_parameters(
        ("distributional_critic", True), ("vanilla_critic", False)
    )
