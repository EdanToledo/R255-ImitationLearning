...
RNDLoss = Callable[[networks_lib.Params, networks_lib.Params, types.Transition], float]


def rnd_update_step(
    state: RNDTrainingState,
    ...
    loss, grads = jax.value_and_grad(loss_fn)(
        state.params, state.target_params, transitions=transitions
    )
...
class RNDLearner(acme.Learner):
    ...
    def __init__(
        ...
        self._update = functools.partial(
            rnd_update_step, loss_fn=loss, optimizer=optimizer
        )
        self._update = utils.process_multiple_batches(
            self._update, grad_updates_per_batch
        )
        self._update = jax.jit(self._update)

        self._get_reward = jax.jit(
            functools.partial(rnd_networks.compute_rnd_reward, networks=rnd_network)
        )
    ...
    def _process_sample(self, sample: reverb.ReplaySample) -> reverb.ReplaySample:
        ...
        rewards = self._get_reward(
            self._state.params, self._state.target_params, transitions
        )
    ...
    def get_variables(self, names: List[str]) -> List[Any]:
        ...
        if learner_names:
            learner_dict = dict(
                zip(learner_names, self._direct_rl_learner.get_variables(learner_names))
            )
    ...
    def save(self) -> GlobalTrainingState:
        return GlobalTrainingState(
            rewarder_state=self._state, learner_state=self._direct_rl_learner.save()
        )
