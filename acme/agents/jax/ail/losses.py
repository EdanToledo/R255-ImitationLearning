...
Loss = Callable[
    [DiscriminatorFn, State, types.Transition, types.Transition, networks_lib.PRNGKey],
    LossOutput,
]
...
def _weighted_average(
    x: jnp.ndarray, y: jnp.ndarray, lambdas: jnp.ndarray
) -> jnp.ndarray:
    return lambdas * x + (1.0 - lambdas) * y
...
def _label_data(
...
) -> Tuple[types.Transition, jnp.ndarray]:
    """Create a tuple data, labels by concatenating the rb and dem transitions."""
    data = tree.map_structure(
        lambda x, y: jnp.concatenate([x, y]), rb_transitions, demonstration_transitions
    )
    ...
    if mixup_alpha is not None:
        ...
        data = tree.map_structure(
            lambda x, y: _weighted_average(x, y, lambdas), data, shuffled_data
        )
...
def _logit_bernoulli_entropy(logits: networks_lib.Logits) -> jnp.ndarray:
    return (1.0 - jax.nn.sigmoid(logits)) * logits - jax.nn.log_sigmoid(logits)


def gail_loss(
    entropy_coefficient: float = 0.0, mixup_alpha: Optional[float] = None
) -> Loss:
...
def pugail_loss(
    ...
    pugail_beta: Optional[float] = None,
) -> Loss:
    ...
    def loss_fn(
        ...
        demo_logits, discriminator_state = discriminator_fn(
            discriminator_state, demo_transitions
        )
        rb_logits, discriminator_state = discriminator_fn(
            discriminator_state, rb_transitions
        )
        ...
        negative_loss = jax.nn.softplus(
            rb_logits
        ) - positive_class_prior * jax.nn.softplus(demo_logits)
        if pugail_beta is not None:
            negative_loss = jnp.clip(negative_loss, a_min=-1.0 * pugail_beta)
...
def _make_gradient_penalty_data(
    rb_transitions: types.Transition,
    demonstration_transitions: types.Transition,
    key: networks_lib.PRNGKey,
) -> types.Transition:
    ...
    return tree.map_structure(
        lambda x, y: _weighted_average(x, y, lambdas),
        rb_transitions,
...
def _compute_gradient_penalty(
    gradient_penalty_data: types.Transition,
    discriminator_fn: Callable[[types.Transition], float],
    ...
    gradients = jnp.concatenate(
        [gradients.observation, gradients.action, gradients.next_observation]
    )
...
def add_gradient_penalty(
    base_loss: Loss, gradient_penalty_coefficient: float, gradient_penalty_target: float
) -> Loss:
    ...
    def loss_fn(
        ...
        gradient_penalty = gradient_penalty_coefficient * jnp.mean(
            _compute_gradient_penalty(
                gradient_penalty_data, apply_discriminator_fn, gradient_penalty_target
            )
        )
