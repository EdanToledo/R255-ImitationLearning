...
class OpenSpielWrapper(dm_env.Environment):
    ...
    def _convert_observation(
        self, open_spiel_timestep: rl_environment.TimeStep
    ) -> List[OLT]:
        observations = []
        for pid in range(self._environment.num_players):
            legals = np.zeros(
                self._environment.game.num_distinct_actions(), dtype=np.float32
            )
            legals[open_spiel_timestep.observations["legal_actions"][pid]] = 1.0
            player_observation = OLT(
                observation=np.asarray(
                    open_spiel_timestep.observations["info_state"][pid],
                    dtype=np.float32,
                ),
                legal_actions=legals,
                terminal=np.asarray([open_spiel_timestep.last()], dtype=np.float32),
            )
        ...
        if self._environment.use_observation:
            return OLT(
                ...
                legal_actions=specs.Array(
                    (self._environment.game.num_distinct_actions(),), np.float32
                ),
