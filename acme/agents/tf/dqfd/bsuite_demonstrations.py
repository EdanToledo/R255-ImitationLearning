...
def _optimal_deep_sea_policy(environment: deep_sea.DeepSea, timestep: dm_env.TimeStep):
    action = environment._action_mapping[
        np.where(timestep.observation)
    ]  # pylint: disable=protected-access
    return action[0].astype(np.int32)


def _run_optimal_deep_sea_episode(
    environment: deep_sea.DeepSea, recorder: DemonstrationRecorder
):
