...
class DiscriminatorModule(hk.Module):
    ...
    def __init__(
        ...
        observation_embedding: Callable[
            [networks_lib.Observation], jnp.ndarray
        ] = lambda x: x,
        name="discriminator",
    ):
        super().__init__(name=name)
    ...
    def __call__(
        ...
        if self._use_action:
            ...
            if isinstance(action_spec, specs.DiscreteArray):
                actions = jax.nn.one_hot(actions, action_spec.num_values)
...
class AIRLModule(hk.Module):
    ...
    def __call__(
        ...
        g_output = DiscriminatorModule(
            ...
            name="airl_g",
        )(observations, actions, next_observations, is_training, rng)
        ...
        return (
            g_output
            + self._discount * h_module(next_observations, (), (), is_training, rng)
...
def make_discriminator(
    ...
    def apply_fn(
        ...
        if logpi_fn is not None:
            logpi = logpi_fn(policy_params, transitions.observation, transitions.action)
