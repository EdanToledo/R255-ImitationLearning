...
def get_actor_core(
    ...
    if (not num_epsilons and evaluation_epsilon is None) or (
        num_epsilons and evaluation_epsilon
    ):
        raise ValueError(
    ...
    def select_action(
        ...
        q_values, recurrent_state = networks.apply(
    ...
    def init(
        ...
        if num_epsilons:
            epsilon = jax.random.choice(
                epsilon_rng, np.logspace(1, 3, num_epsilons, base=0.1)
            )
        else:
