...
def _build_sequence(length: int, step_spec: reverb_base.Step) -> reverb_base.Trajectory:
...
def _zero_pad(sequence_length: int) -> datasets.Transform:
    ...
    def _zero_pad_transform(sample: reverb.ReplaySample) -> reverb.ReplaySample:
        ...
        return reverb.ReplaySample(
            info=sample.info,
            data=reverb_base.Trajectory(**trajectory_steps, extras=trajectory_extras),
        )
...
def _make_adder_config(
    step_spec: reverb_base.Step, seq_len: int, seq_period: int
) -> List[sw.Config]:
...
class R2D2Builder(
            ...
            reverb.Table(
                ...
                signature=sw.infer_signature(
                    configs=_make_adder_config(
                        step_spec, self._sequence_length, self._config.sequence_period
                    ),
                    step_spec=step_spec,
                ),
            )
    ...
    def make_adder(
        ...
        policy: Optional[r2d2_actor.R2D2Policy],
