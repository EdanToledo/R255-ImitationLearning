...
class DiscreteValuedDistributionTest(parameterized.TestCase):
    ...
    def test_constructor(self, batch_shape, event_shape, num_values):
        ...
        values = np.linspace(
            start=-np.ones(event_shape, dtype=float),
            stop=np.ones(event_shape, dtype=float),
            num=num_values,
            axis=-1,
        )
        distribution = distributions.DiscreteValuedDistribution(
        ...
        self.assertEqual(
            distribution.logits_parameter().shape.as_list(), list(logits.shape)
        )
        self.assertEqual(
            distribution.logits_parameter().shape.as_list()[-1], logits.shape[-1]
        )
