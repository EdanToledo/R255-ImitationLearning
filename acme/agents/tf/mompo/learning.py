...
_DEFAULT_INIT_LOG_TEMPERATURE = 1.0
_DEFAULT_INIT_LOG_ALPHA_MEAN = 1.0
_DEFAULT_INIT_LOG_ALPHA_STDDEV = 10.0
...
class MultiObjectiveMPOLearner(acme.Learner):
    ...
    def __init__(
        ...
        if self._objective_names != list(self._policy_loss_module.objective_names):
            raise ValueError(
                "Agent's ordering of objectives doesn't match "
...
def _compute_distributional_critic_loss(
    ...
    for idx, (sampled_q_t_distributions, q_tm1_distribution) in enumerate(
        ...
        sampled_logits = tf.reshape(
            sampled_q_t_distributions.logits, [num_samples, batch_size, -1]
        )  # [N, B, A]
        ...
        critic_loss = losses.categorical(
            q_tm1_distribution, r_t_all[:, idx], discount * d_t, q_t_distribution
        )
        critic_losses.append(tf.reduce_mean(critic_loss))

        # Compute Q-values of sampled actions and reshape to [N, B].
        sampled_q_ts.append(
            tf.reshape(sampled_q_t_distributions.mean(), (num_samples, -1))
        )
...
def _compute_critic_loss(
    ...
    sampled_q_t = tf.reshape(
        sampled_q_t_all, (num_samples, batch_size, num_critic_heads)
    )  # [N,B,C]
