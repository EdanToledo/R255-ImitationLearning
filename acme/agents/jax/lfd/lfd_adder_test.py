...
class LfdAdderTest(absltest.TestCase):
            ...
            if k == 0:
                ...
                action = np.concatenate(
                    [
                        ...
                        np.random.uniform(0.0, 1.0, (action_dim - 2,)),
                    ]
                )
            observation = np.concatenate(
                [
                    np.asarray([episode_type, episode_index], dtype=np.float32),
                    np.random.uniform(0.0, 1.0, (obs_dim - 2,)),
                ]
            )
            if k == 0:
                timestep = dm_env.restart(observation)
            elif k == length - 1:
                timestep = dm_env.termination(0.0, observation)
            else:
                timestep = dm_env.transition(0.0, observation, 1.0)
    ...
    def test_adder(self):
        ...
        min_ratio = target_ratio - self._demonstration_episode_length / float(
            total_count
        )
        max_ratio = target_ratio + self._demonstration_episode_length / float(
            total_count
        )
