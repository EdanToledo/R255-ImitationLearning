...
class ARSLearner(acme.Learner):
    ...
    def _read_results(self):
        while (
            len(self._evaluation_state.received_results)
            ...
            k = PerturbationKey(
                self._training_state.training_iteration, perturbation_id, is_opposite
            )
    ...
    def _update_model(self) -> int:
        ...
        for _, value in self._evaluation_state.received_results.items():
            ...
            normalizer_params = self._normalizer_update_fn(
                normalizer_params, value.observation
            )

        # Keep only top directions.
        top_directions = []
        for i in range(self._config.num_directions):
            reward_forward = self._evaluation_state.received_results[
                PerturbationKey(self._training_state.training_iteration, i, False)
            ].total_reward
            reward_reverse = self._evaluation_state.received_results[
                PerturbationKey(self._training_state.training_iteration, i, True)
            ].total_reward
        ...
        for _, i in top_directions:
            reward.append(
                self._evaluation_state.received_results[
                    PerturbationKey(self._training_state.training_iteration, i, False)
                ].total_reward
            )
            reward.append(
        ...
        policy_params = (
            ...
            / (self._config.top_directions * reward_std)
            * curr_sum
        )
    ...
    def get_variables(self, names: List[str]) -> List[Any]:
        assert names == [ars_networks.BEHAVIOR_PARAMS_NAME] or names == [
            ars_networks.EVAL_PARAMS_NAME
        ]
        if names == [ars_networks.EVAL_PARAMS_NAME]:
            return [
                PerturbationKey(-1, -1, False),
                self._training_state.policy_params,
                self._training_state.normalizer_params,
            ]
        should_sleep = False
