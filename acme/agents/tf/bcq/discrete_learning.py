...
class _InternalBCQLearner(core.Learner, tf2_savers.TFSaveable):
    ...
    def _step(self) -> Dict[str, tf.Tensor]:
        ...
        if tf.math.mod(self._num_steps, self._target_update_period) == 0:
            for src, dest in zip(
                self._q_network.variables, self._target_q_network.variables
            ):
                dest.assign(src)
...
class DiscreteBCQLearner(core.Learner, tf2_savers.TFSaveable):
    ...
    def __init__(
        ...
        **bcq_learner_kwargs,
        ...
        self._bc_logger = bc_logger or loggers.TerminalLogger(
            "bc_learner", time_delta=1.0
        )
        self._bcq_logger = bcq_logger or loggers.TerminalLogger(
            "bcq_learner", time_delta=1.0
        )
